...
def connect_via_jump(jump_host, jump_user, jump_key_path, target_host, target_user, target_password, target_key_path):
    try:
        ...
        if str(target_user) != 'nan':
            ...
            print(target_host + " connected Successfully :")
        else:
...
def main():
    ...
    # 目标服务器信息文件路径
    servers_file = 'db-servers.xlsx'

    # 读取目标服务器信息
    servers = read_servers(servers_file)
    filepath = "C:\PyCharms\PycharmProjects\CheckSystem"
    ...
    with open(txtfilename, 'w') as out_file:
        ...
        for server in servers:
            ...
            try:
                ...
                if target_client:
                    # 执行命令
                    # result = execute_command(target_client, 'cat /etc/auto.nfs_auto_share')
                    # out_file.write(f"服务器: {target_host}\n")
                    # out_file.write(result + '\n\n')
                    # print(result)
                    # result = execute_command(target_client, 'sudo df -h | grep pgvg-pgdata')
                    result = execute_command(target_client, "sudo df -h | grep -E 'pgvg-pgdata|pgvg-backup'")
                    out_file.write(result + '\n\n')
                    print(result)
